...
public class AddToCartServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Integer booknum = Integer.parseInt(request.getParameter("booknum"));
		...
		try {
			...
			if (oldcart != null) {
				newcart.setQuantity(oldcart.getQuantity() + booknum);
				newcart.setFee(oldcart.getFee() + Integer.parseInt(bookfee)*booknum);
				bRet = DAOFactory.getICartDAOInstance().doUpdate(newcart);
			} else {
				newcart.setQuantity(booknum);
				newcart.setFee(Integer.parseInt(bookfee)*booknum);
				bRet = DAOFactory.getICartDAOInstance().doCreate(newcart);
			}
